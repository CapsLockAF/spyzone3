...
const paths = {
	views: {
		src: [
			"./src/views/index.{pug,jade}",
			"./src/views/pages/*.{pug,jade}"
		],
		dist: "./dist/",
		watch: "./src/views/**/*.{pug,jade}"
	},
	styles: {
...
export const libsJs = () => src(paths.libsJs.src)
	.pipe(concat("libs.min.js"))
	.pipe(uglify())
	.pipe(dest(paths.libsJs.dist))
	.pipe(debug({
		"LibsJS": "libsJS files"
	}));
...
export const fontsCopy = () => src(paths.fonts.src)
	.pipe(dest(paths.fonts.dist))
	.pipe(debug({
		"title": "Fonts"
	}));

export const development = series(cleanFiles, smartGrid, parallel(views, styles, libsJs, scripts, images, sprites, favs, fontsCopy),
	parallel(watchCode, server));

export const prod = series(cleanFiles, smartGrid, serverConfig, views, styles, libsJs, scripts, images, sprites, favs, fontsCopy);
